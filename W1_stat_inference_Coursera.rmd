...
samp1 %>%
  summarise(avg = mean(area), samp_med = median(area), 
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means50 %>% summarise(elements = NROW(x_bar))
...
samp1 %>%
  summarise(avg = mean(price), samp_med = median(price), 
            s = sd(price), samp_iqr = IQR(price),
            samp_min = min(price), samp_max = max(price),
            samp_q1 = quantile(price, 0.25),  
            samp_q3 = quantile(price, 0.75))
```
...
sample_means50 <- ames %>%
                    ...
                    summarise(p_x_bar = mean(price))

ggplot(data = sample_means50, aes(x = p_x_bar)) +
  geom_histogram(binwidth = 20)
```
...
ggplot(data = sample_means150, aes(x = p_x_bar)) +
  geom_histogram(binwidth = 20)
```
...
# type your code for this Exercise here, and Run Document
samp2 <- ames %>% sample_n(size = 15)
samp2 %>% summarise(p_x_bar = mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <- ames %>% rep_sample_n(size = 15, reps = 2000, replace = T) %>%
            summarise(avg = mean(price)) 

ggplot(data = sample_means15, aes(x = avg)) +
  geom_histogram(binwidth = 20)

ames %>% summarize(pop_p_mean = mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means150 <- ames %>% rep_sample_n(size = 150, reps = 2000, replace = T) %>%
            summarise(avg = mean(price))

ggplot(data = sample_means150, aes(x = avg)) +
  geom_histogram(binwidth = 20)
...
sample_means50 <- ames %>% rep_sample_n(size = 150, reps = 2000, replace = T) %>%
            summarise(avg = mean(area))

ggplot(data = sample_means150, aes(x = avg)) +
  geom_histogram(binwidth = 20) + labs(title = "150 n")

ggplot(data = sample_means50, aes(x = avg)) +
  geom_histogram(binwidth = 20) + labs(title = "50 n")
```
