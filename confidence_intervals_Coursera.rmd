...
```{r set-seed}
set.seed(656565)                 # make sure to change the seed
```
...
interpreted "typical" to mean. **We can say its normaly distributed with noticable skewness to te high end. The mean should be close to the population mean. If we sample enough samples our sampling mean will be much closer.**
</div>
```{r describe-sample}
# type your code for the Exercise here, and Knit
ggplot(data = samp, aes(x = area)) + geom_histogram(bins = 15) +
  stat_function(fun=dnorm, aes(x = area))
summary(samp$area)
```
...
**Right now 92% include the true mean eventhough theoraticly it should be 95%. I think this is due to the way random process work in computers. If we loop over 50 seeds I think we will be closer to 95%**
</div>
...
6. We would expect 99% of the intervals to contain the true population mean. **I think we need to change seed couple of times maybe the random process in computers is not truly random**
<ol>
<li> **True** </li>
<li> False </li>
</ol>
