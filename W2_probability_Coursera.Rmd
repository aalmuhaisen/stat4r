...
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) + geom_histogram(binwidth = 25)

sfo_feb_flights %>% summarise(mean_ar_del = mean(arr_delay), med_ar_del = median(arr_delay), max_arr_del = max(arr_delay))
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(IQR_ar_del = IQR(arr_delay))
...
nycflights %>% group_by(month) %>% summarise(mean_dep_del = mean(dep_delay))
...
nycflights %>% group_by(month) %>% summarise(med_dep_del = median(dep_delay))

ggplot(data = nycflights, aes(x = dep_delay)) + geom_histogram(binwidth = 25)
...
nycflights %>% group_by(origin) %>% summarise(on_dep_rate = sum(dep_type == "on time") / n())
...
```{r}
#Original code is not formated well, you need to add (position = "fill") for proportion
...
nycflights %>% group_by(tailnum) %>% arrange(desc(avg_speed)) %>% select(tailnum, avg_speed, distance, air_time)
...
summary
summary(lm(distance ~ avg_speed, data = nycflights))
...
nycflights %>% group_by(dep_type2) %>% summarise(on_arr_rate = sum(arr_type == "on time") / n() )
nycflights %>% summarise(on_arr_rate = sum(dep_type2 == "delayed" & arr_type == "on time"))
nycflights %>% summarise(on_arr_rate = sum(dep_type2 == "on time" & arr_type == "delayed"))
nycflights %>% summarise(on_dep_rate = sum(arr_type == "delayed"))
