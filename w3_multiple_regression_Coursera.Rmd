...
NROW(evals$score[evals$score<3])
ggplot(data = evals, aes(score)) + geom_density()
```
...
or mosaic plot). *As we can see below, the average beauty rating of professor decreases as he get older, which is expected.*
</div>
```{r two-vars-eda}
# type your code for the Exercise here, and Knit
ggplot(data = evals, aes(age, bty_avg)) + geom_point() + stat_smooth(method = "lm", se = TRUE)
summary(lm(age ~ bty_avg, data = evals))
```
...
```{r summary-model}
# type your code for the Exercise here, and Knit
bty_score <- lm(score ~ bty_avg, data = evals)
summary(bty_score)
```
...
ggplot(data = bty_score, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")
ggplot(data = bty_score, aes(.resid)) + geom_density()
```
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
ggplot(data = m_bty_gen, aes(.resid)) + geom_density()
```
...
model changed the parameter estimate for `bty_avg`? **After adding `gender`, segnificance of `bty_avg` increased.**
</div>
```{r summary-mlr-model}
# type your code for the Exercise here, and Knit
summary(m_bty_gen)
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_skimmed <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
m_cls_stu <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
could this new information have an impact on any of the conditions of linear regression? **I don't think so.**
</div>
...
with a high evaluation score. **Male young and handsome professor**
</div>

<div id="exercise">
**Exercise:** Would you be comfortable generalizing your conclusions to apply 
to professors generally (at any university)?  Why or why not? **No, because students at that university might have different taste. We need to sample from different universities across the states.**
</div>
