...
ggplot(data = mlb11, aes(at_bats, runs)) + geom_point()
```

<div id="exercise">
**Exercise**: If you knew a team's `at_bats`, would you be comfortable using 
a linear model to predict their number of runs?
*In absence of better explanatory variables, a linear model using at_bats will be the best option.*
</div>
...
5,579 at-bats? Is this an overestimate or an underestimate, and by how much? *The model will overestimate the runs by around 15 runs.*
</div>
...
original_data <- mlb11 %>% filter(at_bats == 5579)
runs1 <- original_data$runs
sm1 <- summary(m1)
coef1 <- sm1$coefficients[1,1]
coef2 <- sm1$coefficients[2,1]
runs1-(coef1+coef2*5579)
```
...
does there seem to be a linear relationship? *Yes*
</div>
```{r runs-vs-sth-else}
...
can you tell? *My variable do better in explaining the variability in runs than at_bats*
</div>
```{r pick-model}
# type your code for the Exercise here, and Knit
